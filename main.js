...
const UseCache = true; // Defina como true para coletar novos dados no localstorage
...
let detectedGesture = ""; // Variável para rastrear o gesto detectado
    ...
    if (predictions.length > 0) {
      ...
      predictions.forEach(async (prediction) => {
        ...
        if (features.length > 0 && model) {
          ...
          detectedGesture = Object.keys(gestures)[gestureIndex] || "";
          if (detectedGesture) {
      ...
      if (!gestureDetected) {
        // Se nenhuma mão foi detectada ou nenhum gesto foi detectado
        document.getElementById("output").textContent = "";
      }
    } else {
      // Se não houver previsões de mãos
      document.getElementById("output").textContent = "";
    }
...
function startRecording(label) {
  ...
  setTimeout(() => {
    ...
    document.getElementById("output").textContent = "";
  }, recordingDuration); // Continua gravando por 5 segundos
}
